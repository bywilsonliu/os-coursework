...
struct arg_struct { //for passing args to threads
    const char *arg1;
    struct TLBentry arg2;
};

void *checkEntry(const char *pageReq, struct TLBentry entry) { //checking each entry in the TLB
    if (*pageReq == entry.pageNum) return (void *) (char*) (entry.frameNum + 1); //hit
    return (void *) (short *) 0; //miss
}


short search_TLB(const char *pageReq, struct Queue *tlb) { //search for a TLB entry by page, returns its frame if found, -1 otherwise
    int k;
    ...
    for (int i = 0; i < k; i++) {
        ...
        frame += (short) (short*) result;
    }
    return frame;
}

void TLB_Add(struct TLBentry entry, struct Queue *tlb) { //append entry to back of queue
    if (tlb->back == -1) {
        tlb->front = 0;
        tlb->back = 0;
